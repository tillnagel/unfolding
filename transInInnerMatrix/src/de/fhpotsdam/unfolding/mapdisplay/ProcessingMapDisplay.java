...
public class ProcessingMapDisplay extends AbstractMapDisplay implements PConstants {
	...
	protected PMatrix3D internalMatrix = new PMatrix3D();
	private boolean firstInternalMatrixCalc = true;
	...
	public ProcessingMapDisplay(PApplet papplet, AbstractMapProvider provider, float offsetX,
		...
		this.setInnerTransformationCenter(new PVector(width / 2, height / 2));
		// this.innerTransformationLocation = new Location(0, 0);

		internalMatrix = new PMatrix3D();
		calcInternalTransformationMatrixTemp();
		calculateMatrix();

		log.debug("center inner: " + getLocationForObjectPosition(width / 2, height / 2));
		log.debug("center normal:" + getLocationForScreenPosition(width / 2, height / 2));
...
//		return getLocationForObjectPosition(innerTransformationCenter.x, 
//				innerTransformationCenter.y);
	 ...
	 */
	private PMatrix3D getInternalTransformationMatrix() {
		return internalMatrix;
	}

	public void calcInternalTransformationMatrixTemp() {
		// PVector itc = getObjectPosForLocation(innerTransformationLocation);
		// calcInternalTransformationMatrixTemp(itc.x, itc.y);

		calcInternalTransformationMatrixTemp(innerTransformationCenter.x,
				innerTransformationCenter.y);
	}

	protected void calcInternalTransformationMatrixTemp(float x, float y) {
		// TODO Integrate tx, scale, innerTransCenter
		
		PMatrix3D invMatrix = new PMatrix3D();
		if (!firstInternalMatrixCalc) {
			internalMatrix.translate(-width / 2, -height / 2);
			
		} else {
			firstInternalMatrixCalc = false;
		}
		invMatrix.apply(internalMatrix);
		invMatrix.invert();
		float originalX = invMatrix.multX(x, y);
		float originalY = invMatrix.multY(x, y);
		//log.debug("original: " + originalX + "," + originalY);
		//log.debug("iMatrix: " + DebugDisplay.getMatrix3DAsString(internalMatrix));

		internalMatrix = new PMatrix3D();
		internalMatrix.translate(x, y);
		internalMatrix.scale((float) sc);
		internalMatrix.translate(-originalX, -originalY);
		internalMatrix.translate(width / 2, height / 2);
