	...
	public void mouseDragged() {
		for (Map map : maps) {
			if (map.isHit(mouseX, mouseY)) {
				...
				// FIXME Drifts after pan, zoom, pan
				PVector center = map.getCenterInScreenCoordinates();
				
				log.debug("centerInScreen: " + center);
				log.debug("centerInLocation: " + map.mapDisplay.getCenterLocation());
				
				// TEST
				// FIXME Only pans minimally around center
				// PVector center = map.mapDisplay
				// .getPointForLocation(map.mapDisplay.innerTransformationLocation);
				//PVector center2 = map.mapDisplay.getObjectPosForLocation(map.mapDisplay.innerTransformationLocation);
