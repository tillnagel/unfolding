...
public class Yahoo {
	...
	public static abstract class YahooProvider extends AbstractMapProvider {
		...
		public static String getZoomString(Coordinate coordinate) {
			...
			return "x=" + (int) coordinate.column + "&y=" + (int) coordinate.row + "&z="
					+ (int) coordinate.zoom;
		}
	...
	public static class RoadProvider extends YahooProvider {
		public String[] getTileUrls(Coordinate coordinate) {
			return new String[] { "http://us.maps2.yimg.com/us.png.maps.yimg.com/png?v="
					+ ROAD_VERSION + "&t=m&" + getZoomString(sourceCoordinate(coordinate)) };
		}
	}

	public static class AerialProvider extends YahooProvider {
		public String[] getTileUrls(Coordinate coordinate) {
			return new String[] { "http://us.maps3.yimg.com/aerial.maps.yimg.com/tile?v="
					+ AERIAL_VERSION + "&t=a&" + getZoomString(sourceCoordinate(coordinate)) };
		}
	}
	...
	public static class HybridProvider extends YahooProvider {
		public String[] getTileUrls(Coordinate coordinate) {
			...
			String over = "http://us.maps3.yimg.com/aerial.maps.yimg.com/png?v=" + HYBRID_VERSION
					+ "&t=h&" + getZoomString(sourceCoordinate(coordinate));
	...
	public static Coordinate fromYahoo(Coordinate coord) {
		...
		return new Coordinate((int) PApplet.pow(2, 18 - coord.zoom - 1), coord.column,
				18 - coord.zoom);
	}

	public static Coordinate toYahoo(Coordinate coord) {
		// Return x, y, z for Yahoo tile column, row, zoom.
		return new Coordinate((int) PApplet.pow(2, coord.zoom - 1) - coord.row - 1, coord.column,
				18 - coord.zoom);
	}
