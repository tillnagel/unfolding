...
public class Map implements MapEventListener {
	...
	public PVector getCenterInScreenCoordinates() {
		...
		return mapDisplay.getScreenPosForLocation(mapDisplay.getCenterLocation());
		
		
//		float[] xy = mapDisplay.getScreenFromObjectPosition(mapDisplay.width/2, mapDisplay.height/2);
...
//			
			...
			mapDisplay.calcInternalTransformationMatrixTemp();
	...
	public void panCenterTo(Location location, boolean tweening) {
		...
		PVector itc2 = mapDisplay.getInnerTransformationCenter().get();
		itc2.sub(new PVector(mapDisplay.width/2, mapDisplay.height/2));
		itc2.mult(factor);
