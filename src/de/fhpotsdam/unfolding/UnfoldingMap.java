 ...
 *
 * (c) 2015 Till Nagel, and others. See http://unfoldingmaps.org/contact.html
 * for full credits. Licensed under MIT License.
 */
public class UnfoldingMap implements MapEventListener {
    ...
    /**
     * UnfoldingMap does not use tweened animation by default. Use
     * {@link #setTweening(boolean)} to switch it on.
     */
    ...
    /**
     * The minimum scale. If set the map cannot be further zoomed in. Use
     * {@link #setZoomRange(float, float)}.
     */
    public float minScale = DEFAULT_MIN_SCALE;
    /**
     * The maximum scale. If set the map cannot be further zoomed out. Use
     * {@link #setZoomRange(float, float)}.
     */
    ...
    /**
     * Indicates whether this map is currently active. May be used for
     * non-direct interactions.
     */
    ...
    /**
     * Internal method to call listening methods in your application for map
     * changes.
     */
     ...
     * Checks whether the given screen coordinates are on this Map.
     *
     * @param screenPosition The position to check.
     * @return True if map is hit, false otherwise.
     */
     ...
     *
     * @param active True if this map is active, false otherwise.
     */
     ...
     *
     * @param location The geographic location with latitude and longitude.
     ...
     *
     * <p>
     * Use {@link #outerRotate(float)} to rotate the map container. Check
     * RotatableMapApp in examples for a comparison.
     * </p>
     *
    ...
    /**
     * Rotates the map to the given angle, that is it is rotated to the target
     * angle.
     *
     ...
     *
     * @param levelDelta The number of levels to zoom in or out.
     */
     ...
     *
     * @param scaleDelta The scale to zoom by.
     */
     ...
     * inner position.)
     *
     * @param x X position to zoom around and pan to (in screen coordinates).
     * @param y Y position to zoom around and pan to (in screen coordinates).
     * @param level Zoom level to zoom to.
     */
     ...
     *
     * @param screenPosition ScreenPosition to zoom around and pan to.
     * @param level Zoom level to zoom to.
     */
     ...
     *
     * @param location The Location to zoom around and pan to.
     * @param zoomLevel Zoom level to zoom to.
     */
     ...
     *
     * @param x X of the position to pan to, in screen coordinates.
     * @param y Y of the position to pan to, in screen coordinates.
     */
     ...
     * centered.
     *
     * @param location The Location to pan to.
     */
     ...
     *
     * @param from ScreenPosition to start from.
     ...
     *
     * @param fromLocation Origin location to pan from.
     * @param toLocation Destination location to pan to.
     */
     ...
     *
     * @param dx Horizontal distance in pixel.
     * @param dy Vertical distance in pixel.
     */
     ...
     *
     * <p>
     * The whole map container is moved. Use one of the panning methods to pan
     ...
     *
     * @param markers The markers to fit.
     */
     ...
     *
     * @param marker The marker to fit.
     */
     ...
     *
     * @param locations A list of locations to fit.
     */
    ...
    // MarkerManagement -----------------------------------------------
    /**
     * Add a MarkerManager to the map. Replaces the default MarkerManager if the
     * given one is the first, and the default one is empty.
     *
     * <p>
     * Use this if you want to handle multiple independent marker groups.
     * Otherwise, simply use UnfoldingMap's marker methods directly (such as
     * {@link #addMarker(Marker)}) which handles this internally with the
     * default MarkerManager.
     * </p>
     *
     * @param markerManager The MarkerManager to add.
     */
    ...
    /**
     * Returns the default MarkerManager, i.e. the first MarkerManager. This is
     * the one used internally by all UnfoldingMap's marker methods.
     *
    ...
    /**
     * Clears all markers from the map. (Removes markers from default and all
     * other MarkerManagers.)
     */
     ...
     *
     * The marker is removed only from the default MarkerManager. If using
     * multiple MarkerManagers, you need to remove the marker on your own.
     *
     ...
     *
     * <p>
     * <em>Note</em>: Uses the default marker manager. If you have more than one
     * marker manager, use {@link MarkerManager#addMarker(Marker)} instead.
     * </p>
     *
     * @param marker The marker or markers to add.
     */
     ...
     * <p>
     * <em>Note</em>: Uses the default marker manager. If you have more than one
     * marker manager, use {@link MarkerManager#addMarkers(List)} instead.
     * </p>
     *
     ...
     *
     * <p>
     * <em>Note</em>: Returns only the markers of the default marker manager. If
     * you have more than one marker manager, use
    ...
    /**
     * Checks whether a marker got hit with the given screen coordinates. Can be
     * used for interactive selection, etc. This only returns the first found
     * marker. Use {@link #getHitMarker(float, float)} to get all hit markers.
     *
     * <p>
     * <em>Note</em>: Returns only the markers of the default marker manager. If
     * you have more than one marker manager, use
    ...
    /**
     * Checks whether multiple markers got hit with the given screen
     * coordinates. Can be used for interactive selection, etc. This returns all
     * found markers. Use {@link #getFirstHitMarker(float, float)} to get only
     * one marker.
     *
     * <p>
     * <em>Note</em>: Returns only the markers of the default marker manager. If
    ...
    /**
     * Gets the current zoom level of the map. Typically ranges from 0 to 18 or
     * higher, depending on the MapProvider.
     *
    ...
    /**
     * Gets the current zoom of the map. Can be used to scale the size of
     * markers.
     *
     * This is the actual scale of the map (not simply a floating zoom level
     * value). It ranges from 0^2 to 18^2 or higher. Use
     * {@link #getZoomFromScale(double)} to convert to floating zoom level.
     *
    ...
    /**
     * Sets the range of map zoom levels. All zoom interactions will be
     * restricted to these levels.
     *
     * @param minZoomLevel The lower zoom level boundary.
     * @param maxZoomLevel The upper zoom level boundary.
     */
    ...
    /**
     * Restricts the area this map can pan to in a rectangular fashion. When
     * map.setTweening is set to true the map animates back to the restricted
     * area.
     *
     * @param topLeftLocation The top left corner of the restricted rectangular
     * area.
     * @param bottomRightLocation The bottom right corner of the restricted
     * rectangular area.
     */
    ...
    /**
     * Restricts the area this map can pan to in a radial fashion. When
     * map.setTweening is set to true the map animates back to the restricted
     * area.
     *
     * @param location The center location of the circular restriction area.
     * @param maxPanningDistance The radius of the circular restriction area in
     * kilometers.
     */
     ...
     *
     * @param x X in object coordinates.
     * @param y Y in object coordinates.
     */
    ...
    /**
     * Sets the offset of the map. Can be used to re-position the map container
     * (not the map content itself).
     *
     * @param x The x of the screen position.
     * @param y The y of the screen position.
     */
    ...
    /**
     * Moves the offset of the map. Can be used to re-position the map container
     * (not the map content itself).
     *
     * @param dx The x distance in screen coordinates.
     * @param dy The y distance in screen coordinates.
     */
     ...
     *
     * @param zoom The zoom value (between 0 and typically 20+).
     ...
     *
     * @param scale The scale value (between 0 and 2<sup>zoom</sup>).
    ...
    /**
     * Sets the tweening flag, i.e. whether the map shall animate between
     * different states.
     *
     * @param tweening Whether or not the map shall animate.
     */
     ...
     *
     * @param bgColor The color in the current colorMode.
     */
