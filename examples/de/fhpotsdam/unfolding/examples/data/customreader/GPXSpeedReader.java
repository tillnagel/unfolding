...
public class GPXSpeedReader extends GeoDataReader {
    ...
    public static List<Feature> loadData(PApplet p, String gpxFilename) {
        final List<Feature> trackFeatures = new ArrayList<Feature>();

        // Load GPX file
        final XML gpx = p.loadXML(gpxFilename);

        Calendar prevTime = null;
        Location prevLocation = null;

        // Create track with all track points
        final ShapeFeature trackFeature = new ShapeFeature(FeatureType.LINES);
        final List<Double> speedList = new ArrayList<Double>();

        XML[] itemXML = gpx.getChildren("trk/trkseg/trkpt");
        for (int i = 0; i < itemXML.length; i++) {

            // Adds location for track point
            final float lat = itemXML[i].getFloat("lat");
            final float lon = itemXML[i].getFloat("lon");
            ...
            try {
                ...
                final Calendar time = StringUtils.parseIsoDateTime(timeStr);
