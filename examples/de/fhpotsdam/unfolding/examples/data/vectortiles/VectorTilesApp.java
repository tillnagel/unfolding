...
/**
 * Interactive vector tiles example to dynamically load and display buildings
 * from OpenStreetMap.
 *
 * Click on the map to load its vector tiles. Press SPACE to clear markers.
 * Press 'A' to load all vector tiles for the current map view.
 *
 * Handles overlapping features, i.e. features returned in multiple vector tiles
 * are shown only once if the ID is the same. This mechanism does not take into
 * account the zoom factor, i.e. the same feature is not loaded anew for another
 * zoom level.
 *
 */
