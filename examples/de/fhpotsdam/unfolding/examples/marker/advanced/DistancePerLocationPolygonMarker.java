...
/**
 * Implements the getDistanceTo method by checking distances to each polygon
 * vertex, and returns the minimum. Thus, in contrast of using the centroid, the
 * results are more like expected.
 *
 * TODO Yet, as only existing vertices are used, it might happen that a vertex
 * from a polygon is still closer as to the nearest point on an edge of the
 * actual nearest polygon.
 */
