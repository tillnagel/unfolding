...
public class CircularMapSnapshot extends MapSnapshot {
    ...
    private PImage getCircularImage(PImage img, int radius, int feather) {
        final PGraphics temp = p.createGraphics(img.width, img.height, PConstants.JAVA2D);
        temp.beginDraw();
        temp.smooth();
        ...
        temp.image(img, 0, 0);
        temp.endDraw();
        final PImage saveArea = p.createImage(temp.width, temp.height, PConstants.ARGB);
        for (int y = 0; y < saveArea.height; y++) {
            for (int x = 0; x < saveArea.width - 1; x++) {
                final int index = x + y * saveArea.width; // Fixed. Original code by amon.owed only worked for squares
                final float d = PApplet.dist(x, y, radius, radius);
                if (d > radius) {
